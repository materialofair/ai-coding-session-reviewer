...
> = (set, get) => ({
  ...
  isProjectFeatured: (projectPath: string): boolean => {
    const { userMetadata } = get();
    const projectMeta = userMetadata.projects[projectPath];
    return projectMeta?.featured === true;
  },

  featureProject: async (projectPath: string) => {
    await get().updateProjectMetadata(projectPath, { featured: true });
  },

  unfeatureProject: async (projectPath: string) => {
    await get().updateProjectMetadata(projectPath, { featured: false });
  },
