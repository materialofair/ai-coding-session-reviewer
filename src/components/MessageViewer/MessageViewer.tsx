...
import { MessageCircle, ChevronDown, ChevronUp, Search, X, Camera, Eye, EyeOff, SlidersHorizontal } from "lucide-react";
...
export const MessageViewer: React.FC<MessageViewerProps> = ({
  ...
  // Collapse adjacent duplicate events that occasionally appear in provider logs.
  // This keeps the timeline readable without mutating original store data.
  const displayMessages = useMemo(() => {
    ...
    const stripVolatile = (value: unknown): unknown => {
      ...
      if (value && typeof value === "object") {
        ...
        for (const key of Object.keys(obj).sort()) {
          ...
          normalized[key] = stripVolatile(obj[key]);
        }
        return normalized;
      }
      if (typeof value === "string") {
        return value.replace(/\bcall_[A-Za-z0-9_-]+\b/g, "call_*");
      }
      return value;
    };
    ...
    const fingerprint = (message: (typeof messages)[number]): string => {
      const contentText = extractClaudeMessageContent(message) ?? "";
      const rawContent =
      ...
      return [
        ...
        contentText.trim(),
        rawContent.trim(),
        safeStringify((message as { toolUse?: unknown }).toolUse),
        safeStringify((message as { toolUseResult?: unknown }).toolUseResult),
      ].join("|");
    };
    ...
    for (const msg of messages) {
      ...
      if (msg.uuid === prev.uuid || fingerprint(msg) === fingerprint(prev)) {
        continue;
      }
      deduped.push(msg);
    }
    ...
    <div className="relative flex-1 h-full flex flex-col">
      ...
      >
        <div className="px-4 py-2.5 space-y-2">
          <div className="flex items-center gap-2">
            {onBack && (
              <button
                ...
                className={cn(
                  ...
                  "bg-muted/45 hover:bg-muted/70 text-muted-foreground hover:text-foreground",
            ...
            <div className="relative flex-1 group">
              <Search className={cn(
                ...
                "text-muted-foreground/60 group-focus-within:text-foreground/70 transition-colors"
              )} />
              <input
                ref={searchInputRef}
                type="text"
                ...
                className={cn(
                  ...
                  "focus:outline-none focus:ring-1 focus:ring-accent/35 focus:border-accent/45",
            ...
            <button
              ...
              className={cn(
                "inline-flex items-center gap-1.5 text-xs px-2.5 py-2 rounded-lg border transition-colors",
                isFocusMode
                  ? "border-accent/45 bg-accent/15 text-foreground"
                  : "border-border/60 bg-card/80 text-muted-foreground hover:text-foreground"
              )}
              title={t("messageViewer.focusMode", { defaultValue: "Focus mode" })}
            >
              {isFocusMode ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
              <span className="hidden md:inline">
                {t("messageViewer.focusMode", { defaultValue: "Focus" })}
              </span>
            </button>

            <button
              type="button"
              onClick={() => setShowToolbarOptions((v) => !v)}
              className="inline-flex items-center gap-1 text-xs px-2.5 py-2 rounded-lg border border-border/60 bg-card/80 text-muted-foreground hover:text-foreground transition-colors"
          ...
          {(showToolbarOptions || !isFocusMode) && (
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center bg-muted/45 rounded-lg p-0.5 border border-border/60">
                <button
                  type="button"
          ...
          {sessionSearch.query && sessionSearch.matches && sessionSearch.matches.length > 0 && (
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-mono tabular-nums text-foreground/80 bg-muted/55 px-2 py-1 rounded-md border border-border/60">
                {sessionSearch.currentMatchIndex + 1}/{sessionSearch.matches.length}
              </span>
              <div className="flex items-center gap-0.5 bg-muted/45 rounded-lg p-0.5 border border-border/60">
                <button
                  type="button"
                  onClick={onPrevMatch}
      ...
      >
        ...
        {/* 메시지 목록 헤더 */}
        {displayMessages.length > 0 && !sessionSearch.query && !isFocusMode && (
